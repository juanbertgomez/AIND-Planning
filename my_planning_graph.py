...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # for this node to be reachable, preconditions need to be satisfied
            if node_a.prenodes.issubset(self.s_levels[level]):
                # add the action nodes to the current node
                a_nodes.append(node_a)
                for node_s in self.s_levels[level]:
                    # make the actual action node a child of the preceding state level
                    node_s.children.add(node_a)
                    # make the actual action node parent of the next state node
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level -1]:
            # load the effects nodes for the preceding actions and add them to the level´s state mode
            for node_s in node_a.effnodes:
                # add the effects note to the set of state nodes
                s_nodes.add(node_s)
                # make the actual state node a child of the preceding action
                node_a.children.add(node_s)
                # make the actual action node parent of the next state node
                node_s.parents.add(node_a)
        # build the state level for the actual effect node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # validate that node_a1 effects aren't canceled by node_a2
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # validate vice-versa effect
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # validate the node_a1 (-) effect don't conflict with  node_a2 (+) preconditions
        for effect in node_a1.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # validate the node_a1 preconditions aren't mutually excluded from node_a2
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # validate that the state have the same symbol and oposite polarty
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # make sure state nodes are not mutually excluded
        for precon_s1 in node_s1.parents:
            for precon_s2 in node_s2.parents:
                if precon_s1.is_mutex(precon_s2):
                    return False
        return True
